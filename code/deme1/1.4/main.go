...
var errInvalidPostArgSpecified = errors.New("More than one positional arguments specified")
...
func parseArgs(w io.Writer, args []string) (config, error) {
	...
	if fs.NArg() > 1 {
		return c, errInvalidPostArgSpecified
	}
	if fs.NArg() == 1 {
		c.name = fs.Arg(0)
	}
...
var usageString = fmt.Sprintf(`Usage: %s <integer> [-h|--help]
	A greeter application which prints the name you Enther <integer>
	number of times.
`, os.Args[0])
...
func main() {
	...
	if err != nil {
		if errors.Is(err, errInvalidPostArgSpecified) {
